...
\noindent
La prima cosa di cui bisogna preoccuparsi quando è definire le porte di ingresso e di uscita. Una volta completata
...
ENTITY gcd IS
    PORT (clock, reset : IN bit;
        xi : IN unsigned (size-1 DOWNTO 0); ?\tikzmark{gcd_input1}?
        yi : IN unsigned (size-1 DOWNTO 0); ?\tikzmark{gcd_input2}?
        out : OUT unsigned (si?\tikzmark{gcd_varsize}?ze-1 DOWNTO 0) ?\tikzmark{gcd_output}?
    );
END gcd;
\end{minted}
\begin{tikzpicture}[remember picture]
    \draw[overlay, ->] (5,0) node [below]{dimensione variabile} -- (pic cs:gcd_varsize);
    \draw[overlay, ->] ($(pic cs:gcd_inputs)+(0,0.1)$) -- ($(pic cs:gcd_inputs)+(2,-0.1)$) node [right]{porte di input};
    \draw[overlay, ->] ($(pic cs:gcd_outputs)+(0,0.1)$) -- ($(pic cs:gcd_inputs)+(2,-0.1)$);
\end{tikzpicture}
...
BEGIN
    ...
    BEGIN
    WAIT UNTIL clock = '1'; ?\tikzmark{S0}?
    x := xi;
    y := yi;
    WHILE (x > 0) LOOP ?\tikzmark{S1}?
        IF (x <= y) THEN
            temp := y;
            y := x;
            x := temp;
...
\end{minted}
\begin{tikzpicture}[remember picture]
\draw[overlay, ->, , red] (0,0) -- (pic cs:S0);
\end{tikzpicture}

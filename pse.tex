...
per trasmettere un segnale in broadcast per indicare, di solito, la posizione ad altri ricevitori bluetooth.
La trasmissione bluetooth si può usare in diversi contesti.
\begin{figure}[h]
    \centering
...
Il \textbf{Communication protocols} indica la parte di comunicazione dal dispositivo locale verso la rete internet. Qui si possono distinguere dispositivo che sono in grado di connettersi alla rete, attraverso WI-FI o rete Cellulare (3g/4g) e dispositivo che non sono in grado di connettersi direttamente a internet e quindi per poterlo fare hanno bisogno di connettersi ad un \textbf{gateway o un hub}, con il quale, è possibile arrivare alla rete esterna.

Per poter capire meglio, immaginiamo di essere in un contesto industriale.
Far comunicare tutte le macchine tra loro o comunque attraverso internet con una rete WI-FI diventa problematico, ecco perchè molte volte si la connesione via cavo restando in locale e successivamente con un gateway si esce verso internet.

Nella parte di \textbf{Communication protocols} troviamo i \textbf{dispositivo gateway} che come accennato precedentemente, servono per "fare da tramite" da una rete locale piccola verso internet. In termini più specifici, il dispositivo gateway permette di interfacciare la rete locale con il protocollo TCP/IP verso internet e qunidi instradare i pacchetti nella destinazione corretta. 

Oltre ai dispositivo gateway ci sono anche gli \textbf{smart gateway} definiti anche \textbf{gateway di confine} che sono dei dispositivo gateway con particolari caratteristiche.
In particolare, uno smart gateway ha una propria memoria interna e delle specifiche appliccazioni embedded che permettono di analizzare i dati direttamente al loro interno prima di instradarli verso un gateway o hub centrale.\\
Nello specifico gli smart gateway permettono:
\begin{itemize}
    \item \textbf{Filtraggio e Aggregazione di dati}: Cercano di dare una semantica ai dati ricevuti e scartare i dati inutili.
...
\end{itemize}
Un'applicazione sul campo di questi smart gateway può essere messa in un contesto di smartcity. Per esempio, un lampione può fare da smart gateway e comunicare i dati da una zona ad un'altra.

Come detto prima, gli smart gateway possono essere anche denominati gateway di confine, questo perchè possono essere impostati come frontiera da un'applicazione chiamata \textbf{Kora} che permette di costruirne uno.

Gli stessi smartphone possono essere considerati dei gateway, infatti, nel momento in cui comunicano con una smartband oppure uno smartwatch, sono loro che ricevono i dati e li inviano alla rete.
...
\end{itemize}
Non tutti i protocolli sono in grado di connettere i dispositivo direttamente a internet. 
Questi protocolli, infatti, hanno bisogno di un gateway per tradurre ciò che trasmettono nella rete locale verso internet (per intenderci serve lo standard TCP/IP).\\
Di seguito elencati alcuni protocolli:
\begin{itemize}
    \item Bluetooth (LE)
    \item RFID
    \item NFC
    \item Zigbee/Z-wave
\end{itemize}
I \textbf{protocolli di applicazione} sono posti ad un livello superiore rispetto al TCP/IP.\\
Questi protocolli essendo a livello superiore, sfruttano comunque il livello TCP/IP, infatti, usano TCP/IP come un protocollo di trasporto per la comunicazione di messaggi specifici per ogni applicazione.\\
Il trasferimento dei dati verso un cloud viene fatto con alcune applicazioni specifiche.
...
\textbf{AMQP} a differenza di MQTT garantisce una minore latenza nel trasferimento dei dati, questo perchè l'uso del protocollo AMQP è stato pensato per applicazioni IoT, infatti, i dati che vengono trasferiti sono di piccole dimensioni e la trasmissione può essere asincrona.

Alla base del protocollo \textbf{MQTT} (Message Queue Telemetry Transport) ci sta il concetto di \textbf{publisher e subscriber}.\\
Usato principalmente per:
\begin{itemize}
    \item Invio di dati con una taglia molto piccola
    \item dispositivi con molte restrizioni hw (embedded system)
    \item Rete con poca banda e alta latenza
\end{itemize}
Il payload dei messaggi essendo binario risulta molto compatto.

Come detto prima, questo protocollo sfrutta il concetto di \textbf{publisher e subscriber}.
Tutti gli IoT dispositivi che sono presenti nella rete si connettono ad un \textbf{broken MQTT}.

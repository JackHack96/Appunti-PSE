...
ENTITY <entity_name> IS
    GENERIC (
        ...
        cnt_dir?\tikzmark{vhdl_constant_name}?             : 
        STRING?\tikzmark{vhdl_type}?      := "up"?\tikzmark{vhdl_value}?
    );
    -- Port declarations
END ENTITY;
\end{sourcecode}
\begin{tikzpicture}[remember picture]
    \draw[overlay, ->] ($(pic cs:vhdl_constant_name)+(0,0.1)$) -- ($(pic 
    cs:vhdl_type)+(1,0.1)$) node [right]{nome della costante};
\end{tikzpicture}

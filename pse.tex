...
ENTITY <entity_name> IS
    GENERIC (
        ...
        cnt_dir             : STRING      := "up"?\tikzmark{vhdl_value}?
    );
    -- Port declarations
END ENTITY;
\end{sourcecode}
\begin{tikzpicture}[remember picture]
    \draw[overlay, ->] ($(pic cs:vhdl_value)+(-7,-0.1)$) -- ($(pic 
    cs:vhdl_value)+(-3,-0.8)$) node [right]{nome della costante};
    \draw[overlay, ->] ($(pic cs:vhdl_value)+(-3,-0.1)$) -- ($(pic 
    cs:vhdl_value)+(0,-0.4)$) node [right]{tipo della costante};
    \draw[overlay, ->] ($(pic cs:vhdl_value)+(0.0,0.1)$) -- ($(pic 
    cs:vhdl_value)+(0.2,0.1)$) node [right]{valore della costante};
\end{tikzpicture}
...
Le varie costanti vengono separate l'una dall'altra tramite sempre tramite il 
punto e virgola (notare che nell'ultima riga è assente).

...
hardware e software, ricordandomi che devo ‘‘riciclare’’ il più possibile le parti già esistenti, e incomincio lo sviluppo o l'eventuale adattamento.
Durante la progettazione della parte hardware, posso decidere di affiancare l'eventuale processore da un co-processore, che svolge un compito
...
\begin{itemize}
	...
	di clock, e considera come importante quello che avviene all'inizio e alla fine di esso,
...
A differenza di quando codifico un algoritmo in software, nel quale mi basta scrivere del codice in 
un certo linguaggi di programmazione, magari \textit{multiplatform} senza preoccuparmi di dove e come
verrà eseguito, quando progetto hardware devo, oltre all'algoritmo in sè, codificare l'architettura
nel quale verrà eseguito.
...
\noindent
La prima cosa di cui bisogna preoccuparsi quando si progetta un modulo hardware è definire le porte di ingresso e
di uscita. Una volta completata l'identificazione delle porte, bisogna individuare l'architettura di riferimento, 
nel nostro caso FSMD, ovvero una macchina a stati finiti combinata con un datapath. Nella definizione della FSM e del DP,
bisogna tenere conto degli eventuali vincoli: se ad esempio mi interessa un circuito veloce posso permettermi un DP
più grande, mentre se voglio che il mio circuito occupi il minor spazio possibile dovrò allargare la FSM.
Riportiamo di seguito un possibile diagramma del modulo \texttt{gcd} e un'implementazione in VHDL (non RTL).
\begin{figure}[h]
    \centering
    \begin{tikzpicture}

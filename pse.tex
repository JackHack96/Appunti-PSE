...
ENTITY gcd IS
    PORT (clock, reset : IN bit;
        xi : IN unsigned (size-1 DOWNTO 0); ?\tikzmark{gcd_inputs}?
        yi : IN unsigned (size-1 DOWNTO 0); ?\tikzmark{gcd_outputs}?
        out : OUT unsigned (si?\tikzmark{gcd_varsize}?ze-1 DOWNTO 0)
    );
END gcd;
\end{minted}
\begin{tikzpicture}[remember picture]
    \draw[overlay, ->] (5,0) node [below]{dimensione variabile} -- (pic cs:gcd_varsize);
    \draw[overlay, ->] (pic cs:gcd_inputs) -- (d)
\end{tikzpicture}
